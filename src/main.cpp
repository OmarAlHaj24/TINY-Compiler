...
struct SymbolTable
{
    ...
    void Insert(const char* name, int line_num, int value=0)
    {
...
// Symbol Table Constructing, add data types and type checking errors
void semantic_analyzer(SymbolTable *symbol_table, TreeNode *node){
    for(int i = 0; i < MAX_CHILDREN; i++){
        if(node->child[i] != 0)
            semantic_analyzer(symbol_table, node->child[i]);
    }
    ...
    if(node->node_kind == ID_NODE){
        ...
        if(node->child[0]->expr_data_type != INTEGER || node->child[1]->expr_data_type != INTEGER){
            throw("Error =( \nOperators Should be Followed and Preceded by Integers!!!");
        }
    }
    else if(node->node_kind == IF_NODE){
        if(node->child[0]->expr_data_type != BOOLEAN){
            throw("Error =( \nIf Satetment Should be Followed by Boolean!!!");
        }
    }
    else if(node->node_kind == REPEAT_NODE){
        if(node->child[1]->expr_data_type != BOOLEAN){
            throw("Error =( \nUntil Should be Followed by Boolean!!!");
        }
    }
    else if(node->node_kind == ASSIGN_NODE){
        if(node->child[0]->expr_data_type != INTEGER && node->child[0]->expr_data_type != BOOLEAN){
            throw("Error =( \nAssignment Should be Followed by Integer or Boolean!!!");
        }
    }
    else if(node->node_kind == WRITE_NODE){
        if(node->child[0]->expr_data_type != INTEGER && node->child[0]->expr_data_type != BOOLEAN){
            throw("Error =( \nWrite Should be Followed by Integer or Boolean!!!");
        }
    }

    if(node->sibling){
        semantic_analyzer(symbol_table, node->sibling);
    }
}
...
// Evaluate mathematical expressions and return their value
int eval(TreeNode *node, SymbolTable *symbol_table){
    if(node->node_kind == NUM_NODE){
        return node->num;
    }
    else if(node->node_kind == ID_NODE){
        return (symbol_table->Find(node->id))->value;
    }
    int num1 = eval(node->child[0], symbol_table);
    int num2 = eval(node->child[1], symbol_table);
...
//see type of node and execute it
void execute(TreeNode *node, SymbolTable *symbol_table){
    if(node->node_kind == READ_NODE){
        cout << "Enter " << node->id <<": ";
        int x; cin >> x;
        symbol_table->Find(node->id)->value = x;
    }
    else if(node->node_kind == WRITE_NODE){
        int num  = eval(node->child[0], symbol_table);
        cout << num << endl;
    }
    else if(node->node_kind == IF_NODE){
        int condition = eval(node->child[0], symbol_table);
        if(condition){
            execute(node->child[1], symbol_table);
        }
        else if(node->child[2]){
            execute(node->child[2], symbol_table);
        }
        
    }
    else if(node->node_kind == REPEAT_NODE){
        int condition = eval(node->child[1], symbol_table);
        do{
            execute(node->child[0], symbol_table);
            condition = eval(node->child[1], symbol_table);
        }while(!condition);
    }
    else if(node->node_kind == ASSIGN_NODE){
        int num = eval(node->child[0], symbol_table);
        symbol_table->Find(node->id)->value = num;
    }
    if(node->sibling){
        execute(node->sibling, symbol_table);
    }
}

int main(){
    try{
        CompilerInfo *compiler_info = new CompilerInfo("input.txt", "output.txt", "debug.txt");
        SymbolTable *symbol_table = new SymbolTable(); 
        TreeNode *root = Parse(compiler_info);
        semantic_analyzer(symbol_table, root);
        PrintTree(root);
        cout << endl;
        symbol_table->Print();
        cout << endl;
        execute(root, symbol_table);
        DestroyTree(root);
